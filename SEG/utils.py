...
def create_adj(coords, adj_threshold=0.2):
    x_coord, y_coord = get_coord(coords)
...
def get_dataloader(image_set, BATCH_SIZE, gpu='cuda:0'):
    ...
    return DeviceDataLoader(DataLoader(dataset, batch_size=BATCH_SIZE, shuffle=True, drop_last=True, num_workers=4, pin_memory=True), device)





def process_separate(raw_data, samples, genes):
